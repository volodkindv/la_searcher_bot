...
def remove_irrelevant_content(content: str) -> str:
    # language=regexp
    patterns = (
        r'(?i)(Карты.*\n|'
...
def delete_sorted_out_all_tags(content: BeautifulSoup) -> BeautifulSoup:
...
def remove_links(content: BeautifulSoup) -> BeautifulSoup:
...
def prettify_soup(content: BeautifulSoup) -> BeautifulSoup:
...
def clean_up_content(init_content: str | bytes) -> str | None:
    ...
    reco_content_text = remove_irrelevant_content(reco_content_text)
    reco_content_text = make_html(reco_content_text)
